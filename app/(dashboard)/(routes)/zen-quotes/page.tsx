    ...
    <div>
      ...
      <div className="mx-8 space-y-4 px-4 lg:px-8 bg-indigo-50 mb-6 py-4 border rounded-xl">
        ...
        <p className="text-muted-foreground ">
          Click the Generate button below to see a Zen quote.
        </p>
      </div>
      ...
      <div className="space-y-4 mx-8 my-6 px-4 lg:px-8">
        ...
        {quotes.length == 0 && !isLoading && (
          <Empty imageUrl="/quote.svg" label="No quote generated." />
        )}
        {quotes.map((quote, index) => (
          <blockquote
            key={index}
            className="p-4 italic border-l-4 bg-neutral-100 text-neutral-600 border-indigo-400 quote"
          >
            <p className="mb-2">{quote.quote}</p>
            <cite className="block text-right font-semibold">
              — {quote.author}
            </cite>
          </blockquote>
        ))}
